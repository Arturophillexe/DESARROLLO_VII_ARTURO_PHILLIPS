...
foreach ($biblioteca as $libro) {
        ...
        $genero = $libro['genero'];
        if (!isset($conteoPorGenero[$genero])) {
        ...
        $autor = $libro['autor'];
        if (!isset($conteoPorAutor[$autor])) {
            $conteoPorAutor[$autor] = 0;
        }
        $conteoPorAutor[$autor]++;
    }

    
    arsort($conteoPorAutor);
    
    $autorConMasLibros = key($conteoPorAutor);

    $reporte = [
        'numero_total_libros' => $totallibros,
        'numero_libros_prestados' => $librosprestados,
        'libros_por_genero' => $conteoPorGenero,
